...
def test_send_tool_get_declaration():
    ...
    assert SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in declaration.parameters.properties
    assert SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in declaration.parameters.required
...
async def test_send_tool_run_async_valid():
    ...
    args = {SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(valid_a2ui)}

    result = await tool.run_async(args=args, tool_context=tool_context_mock)
    assert result == {SendA2uiToClientToolset._SendA2uiJsonToClientTool.VALIDATED_A2UI_JSON_KEY: valid_a2ui}
...
async def test_send_tool_run_async_missing_arg():
    ...
    assert SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in result["error"]

@pytest.mark.asyncio
async def test_send_tool_run_async_invalid_json():
    tool = SendA2uiToClientToolset._SendA2uiJsonToClientTool(TEST_A2UI_SCHEMA)
    args = {SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: "{invalid"}
...
async def test_send_tool_run_async_schema_validation_fail():
    ...
    args = {SendA2uiToClientToolset._SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(invalid_a2ui)}
    result = await tool.run_async(args=args, tool_context=MagicMock())
    assert "error" in result
    assert "Failed to call A2UI tool" in result["error"]
...
def test_converter_convert_valid_response_single():
    ...
    function_response = genai_types.FunctionResponse(
        ...
        response={SendA2uiToClientToolset._SendA2uiJsonToClientTool.VALIDATED_A2UI_JSON_KEY: [valid_a2ui]}
    )
