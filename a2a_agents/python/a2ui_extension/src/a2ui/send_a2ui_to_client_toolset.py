...
Usage Examples:
  ...
  3. Integration with Executor:
    ...
    config = A2aAgentExecutorConfig(
        genai_part_converter=SendA2uiToClientPartConverter()
    )
...
@experimental
def convert_send_a2ui_to_client_genai_part_to_a2a_part(part: genai_types.Part) -> list[a2a_types.Part]:
    if (function_response := part.function_response) and function_response.name == SendA2uiToClientToolset._SendA2uiJsonToClientTool.TOOL_NAME:
        if SendA2uiToClientToolset._SendA2uiJsonToClientTool.TOOL_ERROR_KEY in function_response.response:
            logger.warning(f"A2UI tool call failed: {function_response.response[SendA2uiToClientToolset._SendA2uiJsonToClientTool.TOOL_ERROR_KEY]}")
            return []
        
        # The tool returns the list of messages directly on success
        json_data = function_response.response.get(SendA2uiToClientToolset._SendA2uiJsonToClientTool.VALIDATED_A2UI_JSON_KEY)
        if not json_data:
    ...
    elif (function_call := part.function_call) and function_call.name == SendA2uiToClientToolset._SendA2uiJsonToClientTool.TOOL_NAME:
        return []
